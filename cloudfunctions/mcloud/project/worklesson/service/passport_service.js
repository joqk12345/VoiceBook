...
class PassportService extends BaseProjectService {
	...
	async register(userId, {
		...
		if (!userCheck) {
			...
			let data = {
				...
				USER_LESSON_CHECKIN_CNT: 0
			}
			await UserModel.insert(data);

			let meetService = new MeetService();
			await meetService.editUserMeetLesson(null, userId, 30, LessonLogModel.TYPE.ADMIN_ADD, '', '注册赠送课时');
	...
	async getMyDetail(userId) {
		...
		let fields = 'USER_LESSON_TOTAL_CNT,USER_LESSON_USED_CNT,USER_LESSON_CHECKIN_CNT,USER_MOBILE,USER_NAME,USER_FORMS,USER_OBJ,USER_STATUS,USER_CHECK_REASON'
		...
		if (user) {
			...
			user.USER_LESSON_REMAIN_CNT = totalCnt - checkinCnt;
		}
