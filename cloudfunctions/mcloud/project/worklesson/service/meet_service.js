...
class MeetService extends BaseProjectService {
	...
	async editUserMeetLesson(admin, userId, changeCnt, type, meetId = '', desc = '') {
		...
		let user = await UserModel.getOne(whereUser, 'USER_LESSON_TOTAL_CNT,USER_LESSON_USED_CNT,USER_LESSON_CHECKIN_CNT');
		if (!user) this.AppError('用户不存在');

		// 获取当前课时数
		let currentTotalCnt = user.USER_LESSON_TOTAL_CNT || 0;
		let currentUsedCnt = user.USER_LESSON_USED_CNT || 0;
		let currentCheckinCnt = user.USER_LESSON_CHECKIN_CNT || 0;
		
		// 对于不同操作类型，计算新的课时数
		let newTotalCnt = currentTotalCnt;
		let newUsedCnt = currentUsedCnt;
		let newCheckinCnt = currentCheckinCnt;
		
		if (type == LessonLogModel.TYPE.ADMIN_ADD) {
			// 管理员增加课时：增加总数
			newTotalCnt = currentTotalCnt + changeCnt;
		} else if (type == LessonLogModel.TYPE.ADMIN_REDUCE) {
			// 管理员减少课时：减少总数
			if (currentTotalCnt + changeCnt < currentCheckinCnt) {
				this.AppError('减少后的课时数不能少于已核销的课时数');
			}
			newTotalCnt = currentTotalCnt + changeCnt;
		} else if (type == LessonLogModel.TYPE.USER_APPT) {
			// 预约时：增加已约课时数
			newUsedCnt = currentUsedCnt + Math.abs(changeCnt);
		} else if (type == LessonLogModel.TYPE.USER_CANCEL) {
			// 用户取消预约：减少已约课时数
			newUsedCnt = Math.max(0, currentUsedCnt - Math.abs(changeCnt));
		} else if (type == LessonLogModel.TYPE.ADMIN_CANCEL) {
			// 管理员取消预约：减少已约课时数
			newUsedCnt = Math.max(0, currentUsedCnt - Math.abs(changeCnt));
		} else if (type == LessonLogModel.TYPE.ADMIN_RECOVER) {
			// 管理员恢复预约：增加已约课时数
			newUsedCnt = currentUsedCnt + Math.abs(changeCnt);
		}
		...
		// 验证课时数合理性
		if (newTotalCnt < 0) {
			this.AppError('课时数不能为负数');
		}
		if (newUsedCnt > newTotalCnt) {
			this.AppError('已约课时数不能超过总课时数');
		}
		if (newCheckinCnt > newTotalCnt) {
			this.AppError('已核销课时数不能超过总课时数');
		}

		// 计算剩余课时数（用于日志记录）
		let lastAvailableCnt = currentTotalCnt - currentCheckinCnt;
		let nowAvailableCnt = newTotalCnt - newCheckinCnt;
		...
		let logData = {
			...
			LESSON_LOG_LAST_CNT: lastAvailableCnt, // 变动前可用课时
			LESSON_LOG_NOW_CNT: nowAvailableCnt // 变动后可用课时
		}
		...
		// 修改用户
		let userData = {
			USER_LESSON_TOTAL_CNT: newTotalCnt,
			USER_LESSON_USED_CNT: newUsedCnt,
			USER_LESSON_CHECKIN_CNT: newCheckinCnt,
	...
	// 核销时增加已核销课时数
	async checkinUserMeetLesson(admin, userId, changeCnt, type, meetId = '', desc = '') {
		changeCnt = Number(changeCnt);
		let whereUser = {
			USER_MINI_OPENID: userId,
		}
		...
		let newTotalCnt = currentTotalCnt;
		let newUsedCnt = currentUsedCnt;
		let newCheckinCnt = currentCheckinCnt + Math.abs(changeCnt);
	...
	// 取消核销时减少已核销课时数
	async uncheckinUserMeetLesson(admin, userId, changeCnt, type, meetId = '', desc = '') {
		changeCnt = Number(changeCnt);
		let whereUser = {
			USER_MINI_OPENID: userId,
		}
		...
		let newTotalCnt = currentTotalCnt;
		let newUsedCnt = currentUsedCnt;
		let newCheckinCnt = Math.max(0, currentCheckinCnt - Math.abs(changeCnt));
		...
		let logData = {
			LESSON_LOG_USER_ID: userId,
			LESSON_LOG_TYPE: type,
			LESSON_LOG_CHANGE_CNT: -changeCnt, // 记录为负数
			LESSON_LOG_MEET_ID: meetId,
			LESSON_LOG_DESC: desc,
			LESSON_LOG_LAST_CNT: lastAvailableCnt, // 变动前可用课时
			LESSON_LOG_NOW_CNT: nowAvailableCnt // 变动后可用课时
	...
	async join(userId, meetId, timeMark, formsList) {
		...
		// 预约时增加已约课时数
		await this.editUserMeetLesson(null, userId, 1, LessonLogModel.TYPE.USER_APPT, meetId, '预约《' + meet.MEET_TITLE + '》');
	...
	async checkMeetRules(userId, meetId, timeMark, formsList = null) {
		...
		// 用户课时控制
		let user = await UserModel.getOne({ USER_MINI_OPENID: userId }, 'USER_LESSON_TOTAL_CNT,USER_LESSON_USED_CNT,USER_LESSON_CHECKIN_CNT');
		if (!user) this.AppError('用户不存在');
		
		// 计算剩余课时数 = 总课时数 - 已核销课时数
		let totalCnt = user.USER_LESSON_TOTAL_CNT || 0;
		let usedCnt = user.USER_LESSON_USED_CNT || 0;
		let checkinCnt = user.USER_LESSON_CHECKIN_CNT || 0;
		
		let availableLessons = totalCnt - checkinCnt;
		
		if (availableLessons <= 0) this.AppError('您当前的可用课时数为0，暂无法进行预约');
		
		// 检查是否还有可预约的课时（考虑已预约但未核销的）
		if (usedCnt >= totalCnt) this.AppError('您当前的已预约课时数已达上限，暂无法进行预约');
	...
	async getMeetListByDay(day) {
		...
		let fields = 'meet.MEET_TITLE,meet.MEET_DAYS_SET,meet.MEET_OBJ.cover, DAY_MEET_ID, day, times';
	...
	async getHasDaysFromDay(day) {
		// 查出现有正常的预约项目
		let meetList = await MeetModel.getAll({ MEET_STATUS: ['in', [MeetModel.STATUS.COMM, MeetModel.STATUS.OVER]] }, '_id');
	...
	async cancelMyJoin(userId, joinId) {
		...
		// 取消预约时减少已约课时数
		await this.editUserMeetLesson(null, userId, -1, LessonLogModel.TYPE.USER_CANCEL, join.JOIN_MEET_ID, '取消预约《' + meet.MEET_TITLE + '》');
		
		// 如果是已核销的预约被取消，还需要减少已核销课时数
		if (join.JOIN_IS_CHECKIN == 1) {
			await this.uncheckinUserMeetLesson(null, userId, 1, LessonLogModel.TYPE.USER_CANCEL, join.JOIN_MEET_ID, '取消已核销《' + meet.MEET_TITLE + '》');
		}
