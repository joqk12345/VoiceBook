...
class AdminNewsController extends BaseProjectAdminController {
	...
	async insertNews() {
		...
		// 数据校验 - 与模型定义严格一致
		let rules = {
			title: 'must|string|min:1|max:50|name=标题', // 必填，与模型一致
			cateId: 'must|string|name=分类',
			cateName: 'must|string|name=分类名',
			order: 'int|min:0|max:9999|default=9999|name=排序号', // 可选，有默认值
			desc: 'string|max:200|name=简介', // 可选，允许空字符串
			forms: 'array|name=表单',
		...
		this.logNews('添加了通知《' + input.title + '》');

		// 确保返回完整的数据，包括生成的ID
		return {
			id: result._id || result,
	...
	async previewNews() {
		...
		let service = new AdminNewsService();
		return await service.previewNews(input.id);
	}
	...
	async editNews() {
		...
		let rules = {
			...
			order: 'int|min:0|max:9999|name=排序号', // 改为可选
			desc: 'string|min:0|max:200|name=简介', // 改为可选，最小长度改为0
			forms: 'array|name=表单',
			content: 'array|name=内容', // 添加内容参数
			pic: 'array|name=图片' // 添加图片参数
		};
		...
		this.logNews('修改了通知《' + input.title + '》');
	...
	async createCompleteNews() {
		...
		let rules = {
			...
			cateId: 'must|string|name=分类',
			cateName: 'must|string|name=分类名',
			order: 'int|min:0|max:9999|name=排序号',
			desc: 'string|min:0|max:200|name=简介',
			forms: 'array|name=表单',
			content: 'array|name=内容',
			pic: 'array|name=图片'
		};
		...
		let service = new AdminNewsService();
		let result = await service.createCompleteNews(input);

		this.logNews('创建了完整通知《' + input.title + '》');
