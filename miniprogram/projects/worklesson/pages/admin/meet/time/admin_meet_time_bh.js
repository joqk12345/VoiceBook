	...
	methods: {
		...
		bindTimeDelTap: function (e) {
			...
			if (node.stat.succCnt) {
				...
				pageHelper.showConfirm('该时段已有「' + (node.stat.succCnt) + '人」预约，若选择删除则请先在预约名单里取消所有预约记录，然后再删除！ 若不想取消，可以选择停止该时段', callback);
			} else {
				let callback = () => {
		...
		bindTimeLimitSetCmpt: function (e) {
			...
			if (this.data.curTimesIdx == -1) {
				...
				for (let k = 0; k < days[idx].times.length; k++) {
					...
					days[idx].times[k].limit = Number(this.data.curTimeLimit) ? Number(this.data.curTimeLimit) : 0;
					if (days[idx].times[k].limit <= 0) days[idx].times[k].limit = 0;
				}
			} else {
				...
				node.limit = Number(this.data.curTimeLimit) ? Number(this.data.curTimeLimit) : 0;
				days[idx].times[timesIdx] = node;
				if (node.limit <= 0) node.limit = 0;
			}
