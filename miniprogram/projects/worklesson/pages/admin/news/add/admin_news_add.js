...
Page({
	...
	bindFormSubmit: async function () {
		...
		try {
			...
			// 获取返回的ID信息
			let newsId = result.data ? result.data.id || result.data._id : result.id || result;
			
			if (!newsId) {
				return pageHelper.showModal('创建失败');
			}
			...
			pageHelper.showSuccToast('添加通知成功', 2000, callback);

		} catch (err) {
			console.log(err);
			pageHelper.showModal('创建失败：' + (err.message || err));
		}
